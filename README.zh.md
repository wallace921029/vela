...
打开 <http://localhost:8080>，使用 `.env` 中 `INITIAL_INVITE_CODE` 配置的邀请码（默认 `00000000`）注册第一个管理员账号。该邀请码被使用后，可在 **系统设置 → 邀请码** 页面继续生成新的邀请码。
...
| `INITIAL_INVITE_CODE` | 否 | 首次启动时写入数据库的管理员邀请码，默认 `00000000`，被使用后再修改无效 |
